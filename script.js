...
function searchforecast(searchTerm) {
    ...
    getSearchMethod(searchTerm);
    fetch("https://api.openweathermap.org/data/2.5/forecast?q=" + searchTerm + "&appid=53d2378036e10278637205320c39dd84").then(result => {
        return result.json();
    })
        .then(result => {
            console.log(result);
            ...
            for (var i = 0; i < 5; i++) {
                ...
                const titleEl = $("<h5>").addClass("card-title").text(new Date(result.list[i].dt_txt).toString());

                const tempEL = $("<p>").addClass("card-text").text(`Temp: ${result.list[i].main.temp_max}`);
                const humidityEL = $("<p>").addClass("card-text").text(`humidity: ${result.list[i].main.humidity}`);
                ...
                colEL.append(cardEL);
                // (SPACE) looking at forcast and latching on to child of "row".
